...
// Tests for draft public classes:
    { "zee_proto", zee_proto_test, false, true, NULL },
    { "zee_server", zee_server_test, false, true, NULL },
    { "zee_client", zee_client_test, false, true, NULL },
...
{
    test_item_t *item;
    printf ("Running zee selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef ZEE_BUILD_DRAFT_API // selftest is still in draft
        else
            zee_private_selftest (verbose, item->subtest);
#endif // ZEE_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running zee test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef ZEE_BUILD_DRAFT_API // selftest is still in draft
        else
            zee_private_selftest (verbose, test->subtest);
#endif // ZEE_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
